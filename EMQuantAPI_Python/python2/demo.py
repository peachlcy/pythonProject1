...
# sector Ispandas使用范例
# 001004 全部A股板块  "001004", "2022-05-26", "RowIndex=1,Ispandas=1"
data = c.sector("0.U", "2022-05-26", "RowIndex=1,Ispandas=1")
if (not isinstance(data, c.EmQuantData)):
    print data
else:
    if data.ErrorCode != 0:
...
# 获取专题报表使用范例 "INDEXCOMPOSITION", "", "IndexCode=000300.SH,EndDate=2017-01-13,Ispandas=1
data = c.ctr("INDEXCOMPOSITION", "", "IndexCode=000300.SH,EndDate=2017-01-13,Ispandas=0")
if not isinstance(data, c.EmQuantData):
    print(data)
else:
    if data.ErrorCode != 0:
